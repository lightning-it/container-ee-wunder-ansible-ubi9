...
This document provides the basic user and operator documentation for `container-ee-wunder-ansible-ubi9`.
...
```bash
podman pull quay.io/l-it/ee-wunder-ansible-ubi9:latest
```

Pinned release tags are preferred for repeatable automation:

```bash
podman pull quay.io/l-it/ee-wunder-ansible-ubi9:vX.Y.Z
```
...
podman run --rm -it \
  ...
  --workdir /workspace \
  quay.io/l-it/ee-wunder-ansible-ubi9:latest --help
```
